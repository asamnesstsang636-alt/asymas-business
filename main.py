...
if "📋 Devis" in tab_map:
    with tab_map["📋 Devis"]:
        ...
        if peut_hist_ind or peut_hist_bat: tabs_list.append("📜 Historique") # 1 seul onglet Historique
            ...
            with tabs[tab_idx]:
                if peut_creer_ind:
                    ...
                    for idx, section in enumerate(st.session_state.devis_sections_ind):
                        col_titre1, col_titre2 = st.columns([0.2, 3]) # Titre éditable ligne par ligne
        ...
        # ===== 4. ONGLET HISTORIQUE AVEC 2 SOUS-ONGLETS =====
        if (peut_hist_ind or peut_hist_bat):
            with tabs[tab_idx]:
                ...
                hist_tabs = st.tabs(hist_tabs_list)
                hist_idx = 0

                # --- SOUS ONGLET 1: HISTORIQUE INDUSTRIEL ---
                if peut_hist_ind:
                    with hist_tabs[hist_idx]:
                        try:
                            devis_list = supabase.table('devis').select("*").eq('type', 'Industriel').order("created_at", desc=True).limit(20).execute().data
                        except:
                            devis_list = []
            ...
            hist_idx += 1
                ...
                # --- SOUS ONGLET 2: HISTORIQUE BATIMENT ---
                if peut_hist_bat:
                    with hist_tabs[hist_idx]:
                                ...
                                for f in factures_list_bat:
                                    with st.expander(f"🧾 {f.get('numero')} - {f.get('client')} - NET: {f.get('net_a_payer',0):,.0f} {f.get('devise','USD')}"):
                                        ...
                                        with col2:
                                            ...
                                            """, height=60)
            tab_idx += 1 # <-- FERME L'ONGLET HISTORIQUE PRINCIPAL
