...
div[data-testid="stButton"] button{width:60px!important;height:60px!important;border:3px solid #FFD700!important;border-radius:50%!important;background:#fff!important;box-shadow:0 0 20px #FFD700!important;font-size:11px!important;font-weight:bold!important;color:#000!important;padding:0!important;display:flex;flex-direction:column;align-items:center;justify-content:center;line-height:1.1;}
</style>
...
# 6 boutons cliquables positionnés sur le cercle
st.markdown('<div style="position:absolute;top:calc(45% - 190px);left:50%;transform:translateX(-50%);z-index:10;">', unsafe_allow_html=True)
if st.button("🏪\nCommerce", key="btn_commerce"): st.session_state.selected_module = "Commerce"
st.markdown('</div>', unsafe_allow_html=True)

st.markdown('<div style="position:absolute;top:calc(45% - 145px);right:calc(50% - 225px);z-index:10;">', unsafe_allow_html=True)
if st.button("🚚\nAuto", key="btn_auto"): st.session_state.selected_module = "Auto"
st.markdown('</div>', unsafe_allow_html=True)

st.markdown('<div style="position:absolute;bottom:calc(55% - 145px);right:calc(50% - 225px);z-index:10;">', unsafe_allow_html=True)
if st.button("🧾\nFactures", key="btn_factures"): st.session_state.selected_module = "Factures"
st.markdown('</div>', unsafe_allow_html=True)

st.markdown('<div style="position:absolute;bottom:calc(55% - 190px);left:50%;transform:translateX(-50%);z-index:10;">', unsafe_allow_html=True)
if st.button("🏠\nImmo", key="btn_immo"): st.session_state.selected_module = "Immo"
st.markdown('</div>', unsafe_allow_html=True)

st.markdown('<div style="position:absolute;bottom:calc(55% - 145px);left:calc(50% - 225px);z-index:10;">', unsafe_allow_html=True)
if st.button("📦\nStock", key="btn_stock"): st.session_state.selected_module = "Stock"
st.markdown('</div>', unsafe_allow_html=True)

st.markdown('<div style="position:absolute;top:calc(45% - 145px);left:calc(50% - 225px);z-index:10;">', unsafe_allow_html=True)
...
if st.session_state.selected_module:
    ...
    with st.sidebar:
        ...
        if st.button("🚪 Déconnexion sidebar", use_container_width=True): st.session_state.clear(); st.rerun()
    ...
    #... garde tout ton code des tabs ici tel quel...
