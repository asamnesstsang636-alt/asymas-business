...
if st.session_state.user_role is None: 
    ...
    with col2: 
        ...
        profil = st.selectbox("Utilisateur", ["-- Sélectionner --", "PDG TSANG", "Gérante ASIYA", 
"BASAM"]) 
...
.stButton>button {background-color: #00ff41!important; color: black!important; font-weight: bold; 
border: none;} 
</style> 
""", unsafe_allow_html=True) 
...
# === PDF FACTURE === 
def generer_pdf_facture(numero, type_op, client, details_list, montant, devise, tel_client="+243...", 
periode=""): 
def clean_text(txt): 
...
pdf.cell(90, 6, "", 'LRB', 1, 'L') 
    ...
    pdf.cell(0, 6, "Merci pour votre confiance! ASYMAS BUSINESS - Votre partenaire de croissance", 
ln=True, align="C") 
    pdf.set_text_color(0, 0, 0) 
    pdf.ln(5) 
...
def creer_facture_auto(type_op, client, details, montant, devise="FC", details_list=None, 
tel="+243...", periode=""): 
    numero_facture = f"AS-{datetime.now().strftime('%Y%m%d%H%M%S')}" 
    if details_list is None: 
        details_list = [{"nom": details, "qte": 1, "prix": montant}] 
    pdf_bytes = generer_pdf_facture(numero_facture, type_op, client, details_list, montant, devise, 
tel, periode) 
    try: 
...
"montant": float(montant), "date": str(date.today())} 
        ...
        st.code(repr(e)) 
    try: 
        data_facture = {"numero_facture": str(numero_facture), "type_operation": str(type_op), 
"nom_client": str(client), "details": str(details), "montant": float(montant), "devise": str(devise), 
"date": str(date.today())} 
...
def generer_excel_pro(df_data, titre="Relevé Comptable", total_revenu=0, total_depense=0, 
solde=0): 
    output = io.BytesIO() 
    with pd.ExcelWriter(output, engine='openpyxl') as writer: 
        ...
        worksheet['A4'] = f'Total Revenus: {total_revenu:,.0f} FC | Total Dépenses: {total_depense:,.0f} 
FC | Solde: {solde:,.0f} FC' 
...
if st.session_state.user_role == "UTILISATEUR": 
    ...
    tab1, tab2, tab3, tab4, tab5, tab6, tab7, tab8, tab9 = st.tabs(["📊 Dashboard", "🛍️ Commerce", "📦 
Gestion Stock", "🏠 Immobilier", "🚗 Automobile", "🚘 Gestion Parc", "💰 Comptabilité", "📄 
Factures", "👥 Utilisateurs"]) 
else: 
    tab1, tab2, tab3, tab4, tab5, tab6, tab7, tab8 = st.tabs(["📊 Dashboard", "🛍️ Commerce", "📦 
Gestion Stock", "🏠 Immobilier", "🚗 Automobile", "🚘 Gestion Parc", "💰 Comptabilité", "📄 
Factures"]) 
    tab9 = None 
...
with tab2: 
    ...
    if df_articles.empty: 
        ...
        with col_gauche: 
            ...
            recherche = st.text_input("🔍 Chercher un produit", placeholder="Tape le nom...", 
key="search_c") 
            ...
            if not df_articles_filtre.empty: 
                options = [f"{row['nom_article']} - {row.get('prix_vente',0):,.0f} FC - 
Stock:{row.get('stock','?')}" for _, row in df_articles_filtre.iterrows()] 
                ...
                if c3.button("➕ Ajouter au Panier", width="stretch", key="add_panier_c"): 
                    ...
                    if stock_dispo < qte: 
                        ...
                        st.stop() 
                    st.session_state.panier_commerce.append({"id": int(produit_choisi['id']), "nom": 
str(produit_choisi['nom_article']), "prix": float(produit_choisi.get('prix_vente',0)), "qte": int(qte), 
"stock_dispo": stock_dispo}) 
        ...
        with col_droite: 
            ...
            if st.session_state.vente_finie and st.session_state.pdf_data: 
                ...
                st.download_button(label="📥 Télécharger Facture PDF", data=st.session_state.pdf_data, 
file_name=f"{st.session_state.num_fact}.pdf", mime="application/pdf", width="stretch", 
key="dl_facture_commerce") 
                pdf_b64 = base64.b64encode(st.session_state.pdf_data).decode() 
                st.components.v1.html(f"""<button onclick="printPDF()" style="width:100%; padding:10px; 
background:#00ff41; color:black; font-weight:bold; border:none; border-radius:5px; cursor:pointer; 
margin-top:10px;">🖨️ IMPRIMER LA FACTURE</button><script>function printPDF() {{const pdfData = 
'data:application/pdf;base64,{pdf_b64}'; const win = window.open('', '_blank'); 
win.document.write('<iframe src="' + pdfData + '" width="100%" height="100%" 
style="border:none;"></iframe>'); win.document.close(); setTimeout(() => {{ win.print(); }}, 
1000);}}</script>""", height=60) 
                ...
                for i, item in enumerate(st.session_state.panier_commerce): 
                    with st.container(border=True): 
                        ...
                        new_qte = c1.number_input("QTE", min_value=1, value=item['qte'], 
key=f"qte_panier_c_{i}") 
                ...
                if st.button("💳 Finaliser Vente", type="primary", width="stretch", key="final_c"): 
                    try: 
                        ...
                        with st.spinner("Enregistrement..."): 
                            vente_result = supabase.table("ventes").insert({"total": float(total), "id_utilisateur": 1 
if st.session_state.user_name == "TSANG" else 2 if st.session_state.user_name == "ASIYA" else 3, 
"nom_client": str(nom_client), "telephone_client": str(tel_client)}).execute() 
                            id_vente = vente_result.data[0]['id'] 
                            for i in st.session_state.panier_commerce: 
                                supabase.table("ventes_details").insert({"id_vente": id_vente, "id_article": 
int(i['id']), "quantite": int(i['qte']), "prix_unitaire": float(i['prix']), "sous_total": float(i['prix']) * 
int(i['qte'])}).execute() 
                                supabase.table("articles").update({"stock": i['stock_dispo'] - i['qte']}).eq("id", 
i['id']).execute() 
                            details_list = [{"nom": i['nom'], "qte": i['qte'], "prix": i['prix']} for i in 
st.session_state.panier_commerce] 
                            details_text = ", ".join([f"{i['nom']} x{i['qte']}" for i in 
st.session_state.panier_commerce]) 
                            num_fact, pdf_bytes = creer_facture_auto("Vente Commerce", nom_client, 
details_text, total, "FC", details_list, tel_client) 
        ...
        with st.expander("➕ Ajouter Nouvel Article"): 
            with st.form("form_article", clear_on_submit=True): 
                ...
                if st.form_submit_button("💾 Ajouter Article"): 
                    try: 
                        supabase.table("articles").insert({"nom_article": str(nom), "categorie": str(cat), 
"prix_achat": float(prix_achat), "prix_vente": float(prix_vente), "stock": int(stock)}).execute() 
        ...
        if df_articles.empty: 
            ...
            for _, row in df_articles.iterrows(): 
                with st.expander(f"{row['nom_article']} - {row.get('prix_vente',0):,.0f} FC - 
Stock:{row.get('stock',0)}"): 
                    c1, c2 = st.columns(2) 
                    with c1: 
                        new_nom = st.text_input("Nom", value=row['nom_article'], key=f"nom_{row['id']}") 
                        new_cat = st.text_input("Catégorie", value=row.get('categorie',''), 
key=f"cat_{row['id']}") 
                        new_prix_a = st.number_input("Prix Achat", value=float(row.get('prix_achat',0)), 
key=f"pa_{row['id']}") 
                    with c2: 
                        new_prix_v = st.number_input("Prix Vente", value=float(row.get('prix_vente',0)), 
key=f"pv_{row['id']}") 
                        new_stock = st.number_input("Stock", value=int(row.get('stock',0)), 
key=f"stock_{row['id']}") 
                    c1, c2 = st.columns(2) 
                    if c1.button("✏️ Modifier", key=f"mod_art_{row['id']}", width="stretch"): 
                        try: 
                            supabase.table("articles").update({"nom_article": str(new_nom), "categorie": 
str(new_cat), "prix_achat": float(new_prix_a), "prix_vente": float(new_prix_v), "stock": 
int(new_stock)}).eq("id", int(row['id'])).execute() 
                            st.success("Modifié") 
                            st.cache_data.clear() 
                            st.rerun() 
                        except Exception as e: 
...
if tab4 and st.session_state.user_role in ["PDG", "GERANTE"]: 
    with tab4: 
        ...
        with col1: 
            type_bien = st.selectbox("Type", ["Maison", "Appartement", "Bureau", "Terrain"], 
key="type_bien") 
...
key="btn_facture_immo"): 
            if nom_client and adresse: 
                details_list = [ 
                    {"nom": f"Loyer {type_bien} | Adresse: {adresse} | Durée: {duree_contrat}", "qte": 1, 
"prix": prix}, 
                    {"nom": f"Electricite | {type_bien} - {adresse}", "qte": 1, "prix": electricite}, 
                    {"nom": f"Eau | {type_bien} - {adresse}", "qte": 1, "prix": eau} 
                ] 
                details_text = f"LOUER: {type_bien} | Adresse: {adresse} | Durée Contrat: {duree_contrat} | 
Loyer: {prix} $ | Electricité: {electricite} $ | Eau: {eau} $" 
                periode = date.today().strftime("%B %Y") 
                num_fact, pdf_bytes = creer_facture_auto("Loyer", nom_client, details_text, total_mensuel, 
"$", details_list, tel_client, periode) 
                ...
                st.components.v1.html(f""" 
                    <button onclick="printPDF()" style="width:100%; padding:10px; background:#00ff41; 
color:black; font-weight:bold; border:none; border-radius:5px; cursor:pointer; margin-top:10px;"> 
    ...
    with tab5: 
        ...
        if df_voitures.empty: 
            ...
            with col_gauche: 
                ...
                recherche_voiture = st.text_input("🔍 Chercher une voiture", placeholder="Marque, 
modèle, plaque...", key="search_v") 
                ...
                if recherche_voiture: 
                    mask = (df_voitures['marque'].str.contains(recherche_voiture, case=False, na=False) | 
                            ...
                            df_voitures.get('plaque', pd.Series(dtype=str)).str.contains(recherche_voiture, 
case=False, na=False)) 
                ...
                if not df_voitures_filtre.empty: 
                    ...
                    for _, row in df_voitures_filtre.iterrows(): 
                        details = f"{row['marque']} {row['modele']} - {row.get('annee','')} - {row.get('plaque','')} - 
{row.get('prix',0):,.0f} $ - {row.get('statut','')}" 
...
style="border:none;"></iframe>'); 
                    ...
                    for i, item in enumerate(st.session_state.panier_voiture): 
                        with st.container(border=True): 
                            ...
                            c1, c2 = st.columns([2,1]) 
                            st.session_state.panier_voiture[i]['qte'] = c1.number_input("QTE", min_value=1, 
value=item['qte'], key=f"qte_panier_v_{i}") 
...
key="btn_facture_auto"): 
                        try: 
                            ...
                            with st.spinner("Enregistrement vente..."): 
                                ...
                                num_fact, pdf_bytes = creer_facture_auto("Vente Voiture", nom_client, f"Vente 
{len(st.session_state.panier_voiture)} véhicule(s)", total, "$", details_list, tel_client) 
    ...
    with tab6: 
        ...
        if df_voitures.empty: 
            ...
            for _, row in df_voitures.iterrows(): 
                with st.expander(f"{row['marque']} {row['modele']} - {row.get('plaque','')} - 
{row.get('statut','')}"): 
                    c1, c2, c3 = st.columns(3) 
                    with c1: 
                        new_marque = st.text_input("Marque", value=row['marque'], 
key=f"marque_{row['id']}") 
                        new_modele = st.text_input("Modèle", value=row['modele'], 
key=f"modele_{row['id']}") 
                        new_annee = st.text_input("Année", value=row.get('annee',''), 
key=f"annee_{row['id']}") 
                    ...
                    with c2: 
                        if "plaque" in colonnes_voitures: 
                            new_plaque = st.text_input("Plaque", value=row.get('plaque',''), 
key=f"plaque_{row['id']}") 
                            data_update["plaque"] = str(new_plaque) 
                        if "couleur" in colonnes_voitures: 
                            new_couleur = st.text_input("Couleur", value=row.get('couleur',''), 
key=f"couleur_{row['id']}") 
...
index=boite_options.index(boite_val) if boite_val in boite_options else 0, key=f"boite_{row['id']}") 
                            data_update["boite"] = str(new_boite) 
                        if "prix" in colonnes_voitures: 
                            new_prix = st.number_input("Prix $", value=float(row.get('prix',0)), 
key=f"prix_{row['id']}") 
...
if tab8 and st.session_state.user_role in ["PDG", "GERANTE"]: 
    with tab8: 
        ...
        if df_compta.empty: 
            ...
            categories_fact = ["Toutes"] + list(df_compta_sorted.get('categorie', 
pd.Series(dtype=str)).dropna().unique()) 
            filtre_cat_fact = col_f4.selectbox("📂 Filtrer par Catégorie", categories_fact, 
key="filtre_cat_fact") 
            filtre_client_fact = col_f5.text_input("👤 Nom Client contient", placeholder="Tape un nom...", 
key="filtre_client_fact") 
 
            df_filtre_fact = df_compta_sorted[(df_compta_sorted['date'] >= str(date_debut)) & 
(df_compta_sorted['date'] <= str(date_fin))] 
 
            if filtre_cat_fact!= "Toutes": 
                df_filtre_fact = df_filtre_fact[df_filtre_fact.get('categorie', '') == filtre_cat_fact] 
 
            if filtre_client_fact: 
                df_filtre_fact = df_filtre_fact[df_filtre_fact['description'].str.contains(filtre_client_fact, 
case=False, na=False)] 
            ...
            if len(categories) == 0: 
                ...
                for cat in sorted(categories): 
                    ...
                    with st.expander(f"📁 {cat} - {len(df_cat)} opérations | FC: {total_cat_fc:,.0f} | $: 
{total_cat_usd:,.0f} | €: {total_cat_eur:,.0f}", expanded=True): 
...
df_cat[df_cat['type']=='Dépense']['montant'].sum()) 
                        ...
                        pdf_cat.cell(0, 8, f"Total FC: {total_cat_fc:,.0f} | Total USD: {total_cat_usd:,.0f} | Total 
EUR: {total_cat_eur:,.0f}", ln=True) 
                        pdf_cat.ln(3) 
                ...
                excel_bytes_global = generer_excel_pro(df_filtre_fact, f"Releve Filtré {date_debut}
{date_fin}", 
                ...
                pdf_global.cell(0, 8, f"Total FC: {total_fc:,.0f} | Total USD: {total_usd:,.0f} | Total EUR: 
{total_eur:,.0f}", ln=True) 
                pdf_global.ln(3) 
...
if tab9 and st.session_state.user_role == "PDG": 
    with tab9: 
        ...
        if df_utilisateurs.empty: 
            st.warning("Table 'utilisateurs' vide. Crée-la dans Supabase avec colonnes: id, nom, role, 
password") 
            st.code(""" 
...
('BASAM', 'UTILISATEUR', 'basam2024'); 
            ...
            with st.form("form_passwords", clear_on_submit=False): 
                ...
                new_pass_pdg = c1.text_input("PDG", value=passwords_db.get("PDG", ""), 
type="password", key="pass_pdg") 
                new_pass_gerante = c2.text_input("GÉRANTE", value=passwords_db.get("GERANTE", ""), 
type="password", key="pass_ger") 
                new_pass_user = c3.text_input("UTILISATEUR", value=passwords_db.get("UTILISATEUR", 
""), type="password", key="pass_user") 
                 
                if st.form_submit_button("💾 ENREGISTRER LES MOTS DE PASSE", width="stretch", 
type="primary"): 
                    try: 
                        supabase.table("utilisateurs").update({"password": new_pass_pdg}).eq("role", 
"PDG").execute() 
                        supabase.table("utilisateurs").update({"password": new_pass_gerante}).eq("role", 
"GERANTE").execute() 
                        supabase.table("utilisateurs").update({"password": new_pass_user}).eq("role", 
"UTILISATEUR").execute() 
