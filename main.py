...
if not st.session_state.logged_in:
    ...
    <style>@keyframes pulseRing{0%,100%{transform:translate(-50%,-50%) scale(1);opacity:0.7;}50%{transform:translate(-50%,-50%) scale(1.12);opacity:1;}}</style>
    """, unsafe_allow_html=True)
...
if st.session_state.selected_module is None:
    ...
    # TRUC CRUCIAL : on met dans un st.empty() pour éviter DeltaGenerator
    placeholder = st.empty()
    result = placeholder.html(html_buttons, height=700)
