...
for icon, label, key, x, y in btn_config:
    if st.button(f"{icon}\n{label}", key=f"btn_{key}"):
        if st.session_state.user_permissions.get(key, False) or st.session_state.user_role == "PDG":
            tab_names = {
                "commerce": "🛍️ Commerce", "automobile": "🚗 Automobile", "factures": "📄 Factures",
                "immobilier": "🏠 Immobilier", "stock": "📦 Gestion Stock", "comptabilite": "💰 Comptabilité"
            }
            st.session_state.active_tab = tab_names[key]
            st.rerun()
        else:
