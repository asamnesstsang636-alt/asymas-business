    ...
    with tab_map["👥 Utilisateurs"]:
        ...
        with st.expander("➕ Ajouter Nouvel Utilisateur", expanded=True):
            with st.form("form_user", clear_on_submit=True):
                ...
                perm_acces_bat = col_a1.checkbox("Accès Devis Bâtiment")
                perm_acces_ind = col_a2.checkbox("Accès Devis Industriel")

                st.markdown("**📋 Devis Industriel :**")
                col_i1, col_i2, col_i3 = st.columns(3)
                perm_devis_ind = col_i1.checkbox("Créer")
                perm_devis_ind_dl = col_i2.checkbox("Télécharger")
                perm_devis_ind_pr = col_i3.checkbox("Imprimer")

                st.markdown("**📋 Devis Bâtiment :**")
                col_b1, col_b2, col_b3, col_b4 = st.columns(4)
                perm_devis_bat = col_b1.checkbox("Créer")
                perm_devis_bat_dl = col_b2.checkbox("Télécharger")
                perm_devis_bat_pr = col_b3.checkbox("Imprimer")
                perm_devis_hist = col_b4.checkbox("Historique")
