...
# === ACCUEIL AVEC 6 BOUTONS VISIBLES ===
st.markdown('<div style="position:relative;width:100%;height:700px;background:radial-gradient(ellipse at center 55%, rgba(255,215,0,0.7) 0%, rgba(15,15,15,1) 85%);overflow:hidden;">', unsafe_allow_html=True)
...
# Les 6 boutons avec z-index élevé pour qu'ils passent devant
positions = [
    (90, 190, "🏪\nCommerce", "Commerce"),
    (30, 190, "🚚\nAuto", "Auto"),
    (-30, 190, "🧾\nFactures", "Factures"),
...
for angle, radius, label, module in positions:
    st.markdown(f'<div style="position:absolute;top:50%;left:50%;transform:translate(-50%,-50%) rotate({angle}deg) translate({radius}px) rotate(-{angle}deg);z-index:999;">', unsafe_allow_html=True)
    if st.button(label, key=f"btn_{module}"):
        st.session_state.selected_module = module
    st.markdown('</div>', unsafe_allow_html=True)

st.markdown('<div style="position:absolute;top:20px;right:20px;z-index:999;">', unsafe_allow_html=True)
