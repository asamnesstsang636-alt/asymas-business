...
with tab2:
    ...
    if df_articles.empty:
        ...
        with col_gauche:
            ...
            st.subheader("📦 Rubrique Produit - Scan ou Recherche")
            search_prod = st.text_input("🔍 Scanner QR ou Chercher", placeholder="Scanne code-barres ou tape nom...", key="search_prod_scan").strip()

            df_articles_filtre = df_articles.copy()
            if search_prod:
                search_clean = search_prod.upper().replace("\r", "").replace("\n", "").strip()
                mask = (
    ...
    with tab3:
        ...
        with st.expander("➕ Ajouter Nouvel Article"):
            with st.form("form_article", clear_on_submit=True):
                ...
                code_barres = c3.text_input("Code Barres", placeholder="Scanner ici")
                if st.form_submit_button("💾 Ajouter Article"):
                    try:
                        supabase.table("articles").insert({"nom_article": str(nom), "categorie": str(cat), "prix_achat": float(prix_achat), "prix_vente": float(prix_vente), "stock": int(stock), "code_barres": str(code_barres)}).execute()
            ...
            for _, row in df_articles.iterrows():
                with st.expander(f"{row['nom_article']} - {row.get('prix_vente',0):,.0f} FC - Stock:{row.get('stock',0)}"):
                    ...
                    with c2:
                        ...
                        new_code = st.text_input("Code Barres", value=row.get('code_barres',''), key=f"cb_{row['id']}")
                    c1, c2 = st.columns(2)
                    if c1.button("✏️ Modifier", key=f"mod_art_{row['id']}", width="stretch"):
                        try:
                            supabase.table("articles").update({"nom_article": str(new_nom), "categorie": str(new_cat), "prix_achat": float(new_prix_a), "prix_vente": float(new_prix_v), "stock": int(new_stock), "code_barres": str(new_code)}).eq("id", int(row['id'])).execute()
                            st.success("Modifié")
                            st.cache_data.clear()
                            st.rerun()
                        except Exception as e:
    ...
    with tab5:
        ...
        if df_voitures.empty:
            ...
            with col_gauche:
                ...
                search_qr = st.text_input("QR Code, Plaque, Marque ou Modèle", placeholder="Scanne ici...", key="search_voiture_qr").strip()

                if search_qr:
                    search_clean = search_qr.upper().replace("\r", "").replace("\n", "").strip()
                    try:
                        voiture_trouvee = supabase.table("voitures").select("*").or_(
