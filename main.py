...
html_buttons_locked = html_buttons.replace("cursor:pointer", "cursor:not-allowed;opacity:0.5").replace("<button", "<button disabled")

module_authorizations = {"PDG": ["Commerce", "Auto", "Factures", "Immo", "Stock", "Compta"], "Visiteur": []}
def is_authorized(module_name: str) -> bool:
    return module_name in module_authorizations.get(st.session_state.user_role, [])

# === LOGIQUE EXACTE QUE TU AS DÉCRITE ===

if st.session_state.logged_in:
    st.markdown(f"# ASYMAS BUSINESS - {st.session_state.user_name}")
    
    if st.session_state.selected_module is None:
        # PAGE D'ACCUEIL : on voit les boutons sur le cercle
        st.markdown("### Cliquez sur un module pour démarrer l'opération")
        clicked = components.html(html_buttons, height=720, key="circle_active")
        if clicked:
        ...
        # OUVERTURE SEULE DE L'OPÉRATION CHOISIE
        st.markdown(f"## Module : {st.session_state.selected_module}")
        col1, col2 = st.columns([6,1])
        with col1:
            st.markdown("Sélectionnez une opération dans ce module.")
        with col2:
            if st.button("← Accueil", key="back_home"):
                st.session_state.selected_module = None
                st.rerun()
            if st.button("🚪 Déconnexion", key="logout_mod"):
                st.session_state.clear()
                st.rerun()
    ...
    # AVANT L'ACCÈS : le champ doit être ici, cercle verrouillé
    st.markdown("# ASYMAS BUSINESS")
    st.markdown("### Accès sécurisé")
    st.markdown("Entrez le mot de passe ci-dessous pour débloquer le cercle.")
    components.html(html_buttons_locked, height=720, key="circle_locked")
