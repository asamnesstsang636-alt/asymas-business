...
if isinstance(perms, str):
 ...
 # === CONFIG FLOKI API ===
FLOKI_API_URL = "https://asymas-bot.onrender.com" # Change avec ton URL Render

# === PAGE FLOKI - VERSION CONVERSATION PROPRE ===
if st.session_state.get('page') == 'floki':
    ...
    with col2:
        audio_val = st.audio_input("🎤", key="floki_mic")
    ...
    # Traitement audio - SEULEMENT si nouveau fichier
    elif audio_val is not None and st.session_state.audio_processed:
        ...
        try:
            with st.spinner("🎤 FLOKI écoute..."):
                ...
                if len(audio_bytes) < 2000:
                    st.warning("Parle plus fort + longtemps")
                    st.session_state.audio_processed = True
                    st.stop()
                
                files = {"audio": ("audio.webm", audio_bytes, "audio/webm")}
                res = requests.post(
                    f"{FLOKI_API_URL}/chat/transcribe", 
                    files=files, 
                    timeout=30
                )
                ...
                if res.status_code == 200:
                    ...
                    if not user_input:
                        st.warning("FLOKI n'a rien capté. Réessaie.")
                        st.session_state.audio_processed = True
                        st.stop()
                else:
                    st.error("Serveur FLOKI HS")
                    st.session_state.audio_processed = True
                    st.stop()
                    
        except requests.exceptions.Timeout:
            st.error("Vocal trop long. Max 15 secondes.")
            st.session_state.audio_processed = True
            st.stop()
        except Exception as e:
            st.error(f"Erreur micro: {str(e)[:80]}")
            st.session_state.audio_processed = True
            st.stop()
    ...
    if user_input:
        ...
        with st.spinner("FLOKI analyse..."):
            try:
                ...
                if res.status_code == 200:
                    reponse_floki = res.json().get("reponse", "FLOKI bug. Réessaie.")
                else:
                    reponse_floki = f"FLOKI erreur {res.status_code}"
            except Exception as e:
